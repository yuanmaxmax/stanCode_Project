...
def make_gui(top, width, height, names, draw_names, search_names):
    ...
    entry.focus()
    error_out = tkinter.Text(top, height=2, width=70,
                             name='errorout', borderwidth=2)
    ...
    search_out = tkinter.Text(top, height=2, width=70,
                              name='searchout', borderwidth=2)
    search_out.grid(row=3, column=2, sticky='w')

    entry_button = tkinter.Button(top, text='Show Graph',
                                  command=lambda: handle_draw(entry, canvas, names, error_out, draw_names))
    entry_button.grid(row=0, column=3, sticky='w')
    search_button = tkinter.Button(top, text='Show Search',
                                  command=lambda: handle_search(search_entry, search_out, names, search_names))
    ...
    entry.bind("<Return>", lambda event: handle_draw(
        entry, canvas, names, error_out, draw_names))
    search_entry.bind("<Return>", lambda event: handle_search(
        search_entry, search_out, names, search_names))
...
def handle_draw(entry, canvas, names, error_out, draw):
    ...
    """
    text = entry.get()
    lookups = [name[0].upper() + name[1:].lower()
               for name in text.split()]  # handles casing
    ...
    if invalid_names:
        if len(invalid_names) == 1:
            ...
            out = ', '.join(invalid_names) + \
                ' are not contained in the name database.'
